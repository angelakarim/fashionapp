...
/** Rejected before decoding, to avoid pulling a huge file into memory. */
...
export default function TryOnApp({ displayName }: { displayName: string }) {
  ...
  const handleSelect = async (slot: 1 | 2, file: File) => {
    ...
    // Guard before decoding: a huge file would be read into memory by
    // createImageBitmap. The API enforces its own limit regardless.
    ...
    if (resized.size > MAX_UPLOAD_BYTES) {
      setError("That image is too large to upload. Please try a smaller file.");
      return;
    }
